...
$sql = "SELECT record.BookId, id, RollNo, Textbook, Due_Date, Date_of_Issue, Date_of_Return, DATEDIFF(CURDATE(), Due_Date) AS x 
        FROM LMS.record, LMS.book 
        WHERE Date_of_Issue IS NOT NULL 
            AND Date_of_Return IS NULL 
            AND book.BookId = record.BookId 
            AND Date_of_Issue BETWEEN '$dateStart' AND '$dateEnd'";
